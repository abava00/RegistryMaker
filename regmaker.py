...
class MakeWindow():
    ...
    def setWidget(self):
        ...
        self.material_rootkey_combobox.grid(column= 1, row= 1, pady= 10)

        self.material_regstry_label.grid(column= 0, row= 2, pady= 10)
        self.material_regstry_combobox.grid(column= 1, row= 2, pady= 10)

        self.material_extension_label.grid(column= 0, row= 3, pady= 10)
        self.material_extension_textbox.grid(column= 1, row= 3, pady= 10)

        self.material_exe_label.grid(column= 0, row= 4)
        self.material_exe_button.grid(column= 1, row= 4)

    # ファイル選択ダイアログ
    def ChoiceExeFile(self):
        dir_path = os.path.abspath(os.path.dirname(__file__))
        self.material_exe_filechoice = tkinter.filedialog.askdirectory(initialdir= "C:\Program files")
